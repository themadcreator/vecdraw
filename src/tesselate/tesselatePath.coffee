...
tesselatePath = (data, tesselationScale = 1.0) ->
  lines       = []
  line        = null
  segStart    = null
  segLast     = null
  ...
  tesselateCubic = (startPoint, controlPoint1, controlPoint2, endPoint) ->
    tesselated  = bezier.apply(bezier, [startPoint, controlPoint1, controlPoint2, endPoint, tesselationScale])
    line        = line.concat(tesselated.slice(1))
    bezierPoint = controlPoint2
    segLast     = endPoint

  tesselateQuadratic = (startPoint, controlPoint1, endPoint) ->
    tesselated = quadratic.apply(quadratic, [startPoint, controlPoint1, endPoint, tesselationScale])
    line       = line.concat(tesselated.slice(1))
  ...
  for seg, i in data
    switch seg.instruction
      ...
      when 'S'
        ...
        if (instLast is 'C' or instLast is 'S') and bezierPoint?
      ...
      when 'T'
        startPoint    = segLast
        controlPoint1 = segLast
        if (instLast is 'Q' or instLast is 'T') and quadPoint?
          # Reflect previous control point
          controlPoint1 = [
